...
#
# Newt default-dynamics baselines (no TRM/SimpleTRM/SRM): the default S model (384ld),
# the same with a 16-dim latent, and the same with the XL dynamics MLP ([512, 512] hidden
# dims) toggled via the xl_dynamics_mlp flag. Three runs share one GPU.
...
for SEED in "${SEEDS[@]}"; do
    python3 "$PYTHON_SCRIPT" \
        ...
        wandb_project="TRM Dynamics" \
        wandb_entity="trm-dynamics" \
        wandb_run_name="dmc_newt_s_s${SEED}_16ld" \
        enable_wandb=True \
        seed="$SEED" \
        latent_dim=16 &
done
...
for SEED in "${SEEDS[@]}"; do
    python3 "$PYTHON_SCRIPT" \
        ...
        wandb_project="TRM Dynamics" \
        wandb_entity="trm-dynamics" \
        wandb_run_name="dmc_newt_s_s${SEED}_xl_dynamics_model" \
        enable_wandb=True \
        seed="$SEED" \
        latent_dim=384 \
