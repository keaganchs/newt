...
# (TRM / SimpleTRM / SRM) with 4 H-cycles x 3 L-cycles. No skip connections, no FiLM.
# Three runs share one GPU.
...
for SEED in "${SEEDS[@]}"; do
    python3 $PYTHON_SCRIPT \
        ...
        wandb_run_name="sigreg_trm_s${SEED}_16ld_4h3l_nofilm_noskip" \
        enable_wandb=True \
        H_cycles=4 \
        L_cycles=3 \
        seed="$SEED" \
...
for SEED in "${SEEDS[@]}"; do
    python3 $PYTHON_SCRIPT \
        ...
        wandb_project="TRM Dynamics" \
        wandb_entity="trm-dynamics" \
        wandb_run_name="sigreg_simple_s${SEED}_16ld_4h3l_nofilm_noskip" \
        enable_wandb=True \
        H_cycles=4 \
        L_cycles=3 \
        seed="$SEED" \
...
for SEED in "${SEEDS[@]}"; do
    python3 $PYTHON_SCRIPT \
        ...
        wandb_project="TRM Dynamics" \
        wandb_entity="trm-dynamics" \
        wandb_run_name="sigreg_srm_s${SEED}_16ld_4h3l_nofilm_noskip" \
        enable_wandb=True \
        H_cycles=4 \
        L_cycles=3 \
        seed="$SEED" \
